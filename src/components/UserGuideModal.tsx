...
export const UserGuideModal: React.FC<UserGuideModalProps> = ({
  ...
  const features: FeatureSection[] = [
    ...
    {
      id: 'feature-calendar',
      title: '📅 週間・月間カレンダービュー & 区分・ステータス絞り込み管理',
      category: 'scheduling',
      icon: CalendarIcon,
      summary: '週間・月間カレンダーで予定日ごとの予約件数や時刻・投稿区分を視覚的に一覧管理できます。',
      details: [
        '「📋 リスト表示」「📅 週間カレンダー」「🗓️ 月間カレンダー」をワンクリックで自在に切り替え可能。',
        '【週間カレンダー】横スクロールタイムライン表示（各曜日が横に広がり、その日の予約カードが時系列順に流れる視認性の高いレイアウト）および7列カラム表示に対応。前週・次週への移動や「今週へ」の即時ジャンプ、各曜日ごとの予約カード（時刻・区分・本文・画像）を直感的に確認。',
        '【月間カレンダー】1ヶ月全体のカレンダーマスに、時刻・投稿区分（🚀同時 / 🦋BS / 🌀TH）・本文抜粋を色分けバッジで表示。',
        '「区分フィルター（すべて / 同時投稿 / Bluesky / Threads）」と「ステータスフィルター（すべて / 待機中 / 完了）」で週間・月間ともに瞬時に絞り込み可能。',
        'カレンダー上の投稿や日付から、本文のエディタ復元、予約日時・区分のインライン変更、今すぐ実行、予約解除を即座に操作可能。',
      ],
      tips: '週間カレンダーでは日ごとの配信スケジュールを時系列で把握しやすく、インライン編集で投稿区分や日時の微調整もスムーズに行えます。',
    },
    {
      id: 'feature-presets',
  ...
  const overviewCards: OverviewCard[] = [
    ...
    },
    {
      id: 'ov-calendar',
      icon: CalendarIcon,
      title: '📅 週間横スクロールタイムライン & 月間カレンダー',
      description: '見やすい横スクロールタイムラインと月間カレンダーで予定日・区分・時間を一括管理。よく使う時間プリセットは自由に追加・編集（最大8件）可能です。',
    },
    {
      id: 'ov-images',
      icon: ImageIcon,
  ...
  const faqItems: FaqItem[] = [
    ...
    {
      id: 'faq-calendar',
      question: '週間・月間カレンダーで予約投稿を管理するには？',
      answer: 'ヘッダーの「⏰ 予約管理」ボタンからモーダルを開き、上部の「📅 週間カレンダー」または「🗓️ 月間カレンダー」を選択します。週間カレンダーでは見やすい横スクロールタイムライン表示で曜日ごとの配信カードを確認でき、投稿区分（同時 / Bluesky / Threads）や日時のインライン変更、エディタへの復元、今すぐ実行、予約解除がワンストップで行えます。',
    },
    {
      id: 'faq-presets',
  ...
  const tipsItems: TipItem[] = [
    ...
    },
    {
      ...
      icon: '📅',
      title: '週間カレンダーのインライン変更',
      description: '週間カレンダーのタイムライン上で各予約カードの「区分（同時/BS/TH）」や「配信日時」をその場で変更したり、「今すぐ実行」で配信できます。',
    },
    {
      id: 'tip-backup-json',
      icon: '💾',
